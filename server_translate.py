...
def print_matrix_json_var(hyp, source, target, sid, tid):
  source.append("</s>")
  target.append("</s>")
  links = []
  for ti, target_word_alignment in enumerate(hyp):
    for si,w in enumerate(target_word_alignment):
      links.append((target[ti], source[si], str(w), sid, tid))
  return json.dumps(links,encoding="utf-8")
...
def Load_Model(models, source_file, saveto, save_alignment=True, k=5,
    ...
    for midx in xrange(n_process):
        processes[midx] = Process(
            ...
            args=(queue, rqueue, midx, models, options, k, normalize, verbose, nbest, save_alignment, suppress_unk, return_hyp_graph))
        processes[midx].start()



import codecs
def translate(sentence, saveto, save_alignment=False, k=5,
         normalize=False, n_process=5, chr_level=False, verbose=False, nbest=False, suppress_unk=False, a_json=False, print_word_probabilities=False, return_hyp_graph=False):
    #sys.stderr.write('Translating {0} ...\n'.format(source_file.name))
    #n_samples, source_sentences = _send_jobs(source_file)
    # utility function
    
    #saveto = codecs.open(saveto,'wa','utf-8')
    #saveto = open(saveto,'wa')
    ...
    #n_samples = 1
    #source_sentences = [sentence.strip().split()]
    #source_file = open("../../test/tmp",'r')
    n_samples, source_sentences = _send_jobs([sentence])
    #n_samples, source_sentences = _send_jobs(source_file)
    #####################################
    #this is deque process dont open
    #_finish_processes()
    result = ""
    #print n_samples,source_sentences
    #
    #Alignment initi
    alignmentInfo = ""
            ...
            for j in order:
                ...
                if save_alignment :
                  if a_json:
                    ...
                    alignmentInfo += print_matrix_json_var(alignment[j], source_sentences[i], _seqs2words(samples[j]).split(), i, i+j)
                  else:
                    #save_alignment.write('{0} ||| {1} ||| {2} ||| {3} ||| {4} {5}\n'.format(i, _seqs2words(samples[j]), scores[j], ' '.join(source_sentences[i]) , len(source_sentences[i])+1, len(samples[j])))
                    alignmentInfo += ('{0} ||| {1} ||| {2} ||| {3} ||| {4} {5}\n'.format(i, _seqs2words(samples[j]), scores[j], ' '.join(source_sentences[i]) , len(source_sentences[i])+1, len(samples[j])))
                    #print_matrix(alignment[j], save_alignment)
                    alignmentInfo += print_matrix(alignment[j])
		...
		renderer.wordify(word_idict_trg)
            ...
            if save_alignment:
              if a_json:
                ...
                alignmentInfo += print_matrix_json_var(alignment, source_sentences[i], _seqs2words(trans[0]).split(), i, i)
              else:
                #save_alignment.write('{0} ||| {1} ||| {2} ||| {3} ||| {4} {5}\n'.format(
                ...
                alignmentInfo += print_matrix_var(alignment[j])

    sys.stderr.write('Done\n')
    result_ALL = dict()
    result_ALL['translation'] = result
    result_ALL['align'] = {"source_align":sentence,"target_align":result,"alignInfo":alignmentInfo}
    return result_ALL
